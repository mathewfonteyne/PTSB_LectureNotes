/* 
    ? Promises
        ...
        A promise allows us to "wait" for our results (regardless is we get a positive response or a negative rejection).

        3 states:
            - Pending, ...in the process of getting the result
            - Fulfilled, ...successfully got the result
            - Rejected, ...not able to get the result
...
// let data = getData(); // The value comes "late" by 3 seconds.
...
function returnData() {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if(true) {
                resolve("Some data from resolve promise!")
            } else {
