...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const user = new User({
      ...
      // password: req.body.password, // Should not store plain text password in the db
    ...
    // Create a token using the sign method of jwt, (payload, message, expiration/options)
    // The payload should be the user ID, and the secret message should eventually be in the .env
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    // .compare() method from bcrypt takes in 2 parameters (password from req.body, and hashed password from DB)
