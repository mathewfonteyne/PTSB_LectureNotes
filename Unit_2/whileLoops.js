...
console.log(`Spider-Man lands in ${structure}!`);
...
/* 
    ? Do While Loop: also loops through a block of code while a specified condition is true BUT...
        ...
        - Works just like while loop with the exception that it will run our code block at least once.
*/
...
// Mock switching accounts from deactivated, use a boolean
...
do {
    // Write a message toi show inactive
    console.log(
        `acct${acctNum} is currently inactive: set to ${accounts[`acct${acctNum}`]}.`);

      // Activating message & set account to true
      console.log("Activating...");
      accounts[`acct${acctNum}`] = true;

      // Confirmation message
      console.log(
        `acct${acctNum} is now active: set to ${accounts[`acct${acctNum}`]}.`);
